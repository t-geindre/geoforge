...
func NewGame() *Game {
	...
	//inland := noise.NewSmoothstep(landMask, 0.65, 0.95)
	//
	...
	height := noise.NewOpenSimplex(seed)
	height = noise.NewFbm(height, 60, 0.5, 2.0)

	return &Game{
		cam:      cam.NewWheelZoom(cam.NewMousePan(cam.NewCamera())),
		world:    world.NewWorld(256, 2, 1024, height),
